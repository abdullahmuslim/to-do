...
function TimePicker(props) {
  ...
  const [hoursHandler, setHoursHandler] = useState(null)
  
  const handleHours = event => {
    setHoursHandler(setTimeout(recordHour, 800))
  }
  const [minHandler, setMinHandler] = useState(null)
  
  const handleMin = event => {
    setMinHandler(setTimeout(recordMinute, 1000))
  }
  let timedHourRecord;
  let timedMinRecord;
  let timedMerRecord;
